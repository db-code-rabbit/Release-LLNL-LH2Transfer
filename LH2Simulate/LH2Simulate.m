...
%
%%  Activate or deactivate the corrected Nusselt number expressions
...
function dxdt = LH2dxdt(P,t,x)
    ...
    % Added by DB : Initialization of these variables to avoid warning messages
    l_V1    =zeros(P.nV1,1); l12_V1 =l_V1; Tv1 =l_V1; % Added by DB
    l_L1    =zeros(P.nL1,1); l12_L1 =l_L1; TL1 =l_L1;  % Added by DB
    l12_L2  =zeros(P.nL2,1); l_L2 =l12_L2; TL2 =l_L2;  % Added by DB
    l12_V2  =zeros(P.nV2,1); l_V2 =l12_V2; % Added by DB
    duL1dt  =l_L1; duv1dt =l_V1; duL2dt =l12_L2; duv2dt =l12_V2; % Added by DB
...
%%
    ...
    fprintf(2, "===-----------------------------------===\n");
    fprintf(2, "VAPOR LAYERS : STORAGE TANK RESULTS\n");
    for i = 1:P.nV1-1
        disp("===---");
        if uv1(i) <0
            uv1(i) =0;
        ...
        fprintf("Vapor Pressure in [kPa]: %g\n", (pv1/1000));
        fprintf("Internal Energy in [J/kg]: %g\n", uv1(i));
        Tv1(i) = refpropm('T','P',pv1/1000,'U',uv1(i)/1.5,'PARAHYD'); % temperature of the vapor layers  in (ST)
        fprintf("Temperature in [K]: %g\n", Tv1(i));
    end
    fprintf(2, "===-----------------------------------===\n");
...
%%
   ...
   try
    ...
    fprintf(2, "===-----------------------------------===\n");
    fprintf(2, "EXTERNAL TANK : RESULTS INSIDE VAPOR LAYERS\n");
    for i = 1:P.nV2-1
        disp("===---");
        if uv2(i) <0
            uv2(i) =0;
        ...
        fprintf("Vapor Pressure in [kPa]: %g\n", (pv2/1000));
        fprintf("Internal Energy in [J/kg]: %g\n", uv2(i));
        Tv2(i) = refpropm('T','P',pv2/1000,'U',uv2(i),'PARAHYD'); % temperature of the vapor layers in (ET)
        fprintf("Temperature in [K]: %g\n", Tv2(i));
    end
    fprintf(2, "===-----------------------------------===\n");
...
%%
    ...
    %Added by DB for information 
    fprintf(2,"\nValve LH2Control\n"); %for information
    disp(U); %for information
    fprintf(2,"LH2 level inside ET, hL2 [m]: %g\n\n", hL2); %for information

    if (IFmodificationIs ==false)
        rhotr =rho_L1;
    else %Added by DB 
        Tb =(TL1(P.nL1)+TL2(P.nL2))/2.0; 
	    rhotr =refpropm('D','T',Tb,'Q',0,'PARAHYD');
    end

	% calculate transmission line parameters
    %apipe = 2*pi*(P.DPipe/2)^2*sqrt(rho_L1*P.DPipe/2/P.LPipe/P.f);
    apipe =2*pi*(P.DPipe/2)^2*sqrt(rhotr*P.DPipe/2/P.LPipe/P.f);% modified by DB with rhotr !!

    % calculate valve area and lambda for fill valve
    AE = (2*pi*(P.dE/2)^2);
    lambdaE = U.lambdaE;
    %alphaE = AE*sqrt(2*rho_L1/P.kE);
    alphaE = AE*sqrt(2*rhotr/P.kE); % modified by DB by using rhotr !!
...
%%
    ...
    if (IFmodificationIs ==true) %
        Ra =abs(P.g* beta_v*(Tw2-Tv2(P.nV2))*(P.H-hL2)^3/nuv2^2)*Pr;    % Modified by DB
        Nu =VerticalPlates('Churchill',Ra,Pr,P.H-hL2,2*P.R2);           % Modified by DB 
    else 
...
%
    if (IFmodificationIs ==true)
        Ra_L_side =abs(P.g*beta_L*(Tw2-TL2(P.nL2))*(hL2)^3/nuL2^2)*Pr_L;    % Modified by DB
        Nu_L_side =VerticalPlates('Churchill',Ra_L_side,Pr_L,hL2,2*P.R2);   % Modified by DB
    else
...
%
    if (IFmodificationIs ==true)
        Ra_L_bottom = abs(P.g*beta_L*(Tw2-TL2(P.nL2))*(2*P.R2)^3/nuL2^2)*Pr_L; % Modified by DB
    else 
...
%
    ...
    if (IFmodificationIs ==true)
        hWL2_bottom = Nu_L_bottom*P.kappa_L/(2*P.R2); % Modified by DB
    else 
...
%%  
    ...
    %%%%%%%%
%%
    % variation of mass of vapor in (ST)
    Jv1 = Jboil - Jvvalve1 - Jcd1;
    % variation of mass of liquid in (ST)
    JL1 = -Jtr - Jvap + Jcd1; 

    % variation of mass of vapor in (ET)
    Jv2 = (P.ratio_top_bottom) * Jtr  - Jvvalve2 - Jcd2;
    % variation of mass of liquid in (ET)
    JL2 = (1-P.ratio_top_bottom) * Jtr + Jcd2;          
...
%% Added by DB
    if (IFmodificationIs ==true) % Added by DB  
        if TL2(P.nL2) <14
            TL2(P.nL2) =14;
        end
        if TL2(P.nL2) >32
            htr_L2 =P.c_L*TL2(P.nL2); 
        else
            htr_L2 =refpropm('H','T',TL2(P.nL2),'Q',0,'PARAHYD');
        end
    end	
%%
   ...
   %
    if (IFmodificationIs ==false)
        QdotV2 = QdotWV2 - QdotVS2 - pdV2 ...   %+ P.ratio_top_bottom*Jtr * X_v * h_X_v ... %(htr+0.5*vtr^2)... % term added for top fill
            ...
            - Jcd2*hcd2;
    else % Modified by DB by replacing htr_L with htr_L2
        QdotV2 = QdotWV2 - QdotVS2 - pdV2 ...   %+ P.ratio_top_bottom*Jtr * X_v * h_X_v ... %(htr+0.5*vtr^2)... % term added for top fill
            + P.ratio_top_bottom * Jtr * (htr_L2+0.5*vtr^2-qh2)... % term added for top fill
            - Jvvalve2*(hvalve2 + 0.5*vv2^2) ...
            - Jcd2*hcd2;
    end
    ...
    if (IFmodificationIs ==false)
        ...
        QdotL2 = QdotWL2 - QdotLS2 + pdV2 ...
               ...
               + Jcd2*hcd2;
    else % Modified by DB by using htr_L2
        QdotL2 = QdotWL2 - QdotLS2 + pdV2 ...
               + (1-P.ratio_top_bottom)*Jtr*(htr_L2+0.5*vtr^2) ...%+ P.ratio_top_bottom * Jtr* (1-X_v)  * h_X_L ...
               + Jcd2*hcd2;
    end
...
%
    if (IFmodificationIs ==false)
        ...
        PPP =  P.ratio_top_bottom * Jtr * (htr_L2+0.5*vtr^2-qh2);
    end
 %
...
%
    if (IFmodificationIs ==false)
        ...
        UUU = + (1-P.ratio_top_bottom)*Jtr*(htr_L2+0.5*vtr^2); 
    end
%
    VVV =  + Jcd2*hcd2;
    WWW = QdotV2;
...
%%
    ...
    % Solve ODE equation by using ode15s ...
...
%%  
    ...
    fprintf("%s", "EXTRACT STATE VARIABLES ...");
...
%%
    ...
    %
    % ============    
    % [value,isterminal,direction] = VentEvents(x,P,ventstate)
    % ============
    %
    %%%%%%%%
    %%%%%%%%
