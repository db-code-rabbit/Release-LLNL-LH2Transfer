...
% here : (ST : STORAGE TANK)  or 1 is trailer (horizontal cylinder)
%        (ET : EXTERNAL TANK) or 2 is station storage (vertical cylinder)

clear LH2Model;

% test
initPetitPas =testPetitPas();
...
%% Creating of a Input and Initial Data Structure named LH2Model
...
LH2Model.pct_hL20 = 0.01; %0.01 ???              % initial level of liquid, measured in inH2O but reported as a fraction (i.e. 0.5 is 5 out of 10 inH2O). Value should be between >0 and 1 (=0 may trigger errors)
...
if (initPetitPas ==true)
    LH2Model.STVentState = 0;               % initial ST vent state (starts closed)
else
    LH2Model.STVentState = 1;               % initial ST vent state (starts closed)
end
...
if (initPetitPas ==true)
    ...
    LH2Model.ETVentState = 1; %% ???     % initial ET vent state (starts closed) 
end
...
if (initPetitPas ==true)
    LH2Model.ratio_top_bottom=0.; %???  % ratio between top and bottom fill to (ET). 0.5 = 50% of liquid goes to top. Values between 0.01 and 0.03 are best...
else
    LH2Model.ratio_top_bottom=0.01;%0 %???  % ratio between top and bottom fill to (ET). 0.5 = 50% of liquid goes to top. Values between 0.01 and 0.03 are best...
end

%% Solver options
%
LH2Model.tFinal = 30*60;                % [s] final time   60*60
